...
class _FakeClient:
    def __init__(
        ...
        *,
        ...
        self.kill_session_calls: list[tuple[str, str]] = []
    ...
    async def kill_session(self, *, project_id: str, session_id: str) -> None:
        self.kill_session_calls.append((project_id, session_id))
...
@pytest.mark.asyncio
async def test_kill_requests_session_kill_with_yes(monkeypatch) -> None:
    fake_client = _FakeClient()
    printed: list[str] = []
    ...
    monkeypatch.setattr(sessions, "get_client", fake_get_client)
    monkeypatch.setattr(sessions, "resolve_project_id", fake_resolve_project_id)
    monkeypatch.setattr(sessions, "print", printed.append)

    await sessions.kill(project_id="project-1", session_id="session-1", yes=True)

    assert fake_client.kill_session_calls == [("resolved-project", "session-1")]
    assert fake_client.closed is True
    assert printed == ["Kill requested for session session-1"]


@pytest.mark.asyncio
async def test_kill_skips_api_when_confirmation_declines(monkeypatch) -> None:
    fake_client = _FakeClient()
    ...
    monkeypatch.setattr(sessions, "get_client", fake_get_client)
    monkeypatch.setattr(sessions.typer, "confirm", lambda *_args, **_kwargs: False)

    with pytest.raises(sessions.typer.Exit):
        await sessions.kill(project_id="project-1", session_id="session-1", yes=False)

    assert fake_client.kill_session_calls == []
