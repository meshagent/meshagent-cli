...
def _setup_command(
    ...
    async def runner():
        ...
        setup_api_url = resolve_api_url(api_url=api_url)

        async def list_setup_projects() -> list[SetupProject]:
            client = await get_client(api_url=setup_api_url)
            try:
            ...
            return [
                SetupProject(id=project.id, name=project.name)
                for project in projects._parse_listed_projects(
                    response,
        ...
        async def create_project_from_name(project_name: str) -> str:
            client = await get_client(api_url=setup_api_url)
            try:
                return await projects._create_project_id(
                    client,
