...
@pytest.mark.asyncio
async def test_deploy_template_pack_applies_runtime_container_override(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    captured: dict[str, object] = {}
    source_dir = tmp_path / "contact-form"
    source_dir.mkdir()
    (source_dir / ".meshagent").mkdir()
    (source_dir / "Dockerfile").write_text(
        """
...
""".strip(),
    ...
    (source_dir / ".meshagent" / "deploy.yaml").write_text(
        """
version: v1
kind: ServiceTemplate
metadata:
...
""".strip(),
    ...
    class _FakeAccountClient(_FakeDeployConfigClient):
        async def list_room_services(self, *, project_id: str, room_name: str):
            captured.setdefault("list_room_services", []).append(
                (project_id, room_name)
            )
    ...
    await image.deploy_image(
        project_id="project-1",
        room="room-1",
        tag="registry.meshagent.com/repo/contact-form:1",
        pack=str(source_dir),
        context_path="/context",
        dockerfile_path="/context/Dockerfile",
        optimize=False,
    ...
    service_spec = created_service[2]
    assert service_spec.container is not None
    assert service_spec.container.image == IMAGE_RUNTIME_BASES["python"].base_image
    assert service_spec.container.command == "python -m server"
    assert service_spec.container.working_dir == IMAGE_RUNTIME_MOUNT_PATH
    assert service_spec.container.template == "agent"
    assert service_spec.container.private is False
    assert service_spec.container.environment is not None
    assert [
        (env_var.name, env_var.value) for env_var in service_spec.container.environment
    ] == [
        ("CONTACT_FORM_TO", "ops@example.com"),
        ("APP_MODE", "production"),
    ]
    assert service_spec.container.storage is not None
    assert service_spec.container.storage.images is not None
    image_mount = service_spec.container.storage.images[0]
    assert image_mount.image == "registry.meshagent.com/repo/contact-form:1"
    assert image_mount.path == IMAGE_RUNTIME_MOUNT_PATH
    assert image_mount.subpath == IMAGE_RUNTIME_MOUNT_SUBPATH
    assert image_mount.read_only is True
    assert captured["deleted_image"] == "registry.meshagent.com/repo/contact-form:1"
    assert "restarted_service_id" not in captured
    assert captured["room_client_closed"] is True
    assert captured["account_client_closed"] is True
