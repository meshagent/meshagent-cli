...
const contentDisplayPath = "src/dev-content.json";
const contentPath = path.join(__dirname, contentDisplayPath.split("/").pop());
const toolkitName = "meshagent.create.typescript-content";
...
class TypeScriptCreateContentTool extends Tool {
  constructor() {
    super({
      name: "create",
      title: "Create local web content",
      description: "Creates a local TypeScript content record rendered by the dev app.",
      inputSchema: {
        type: "object",
...
class TypeScriptUpdateContentTool extends Tool {
  constructor() {
    super({
      name: "update",
      title: "Update local web content",
      description: "Updates a local TypeScript content record rendered by the dev app.",
      inputSchema: {
        type: "object",
        required: ["id"],
        additionalProperties: false,
...
class TypeScriptSearchContentTool extends Tool {
  constructor() {
    super({
      name: "search",
      title: "Search local web content",
      description: "Searches content records currently available to the local TypeScript app.",
      inputSchema: {
        type: "object",
        required: ["query"],
        additionalProperties: false,
...
class TypeScriptContentToolkit extends Toolkit {
  constructor() {
    super({
      name: toolkitName,
      title: "TypeScript Local Content Toolkit",
      description: "Local-only create, update, and search tools for the TypeScript dev app content.",
      tools: [
        new TypeScriptCreateContentTool(),
        new TypeScriptUpdateContentTool(),
        new TypeScriptSearchContentTool(),
      ],
    });
  }
}
...
async function runDevContentToolkit(focus, existingRoom) {
  ...
  const hostedToolkit = await startHostedToolkit({
    room,
    toolkit: new TypeScriptContentToolkit(),
    public_: true,
  });
  ...
  try {
    ...
    const created = await room.agents.invokeTool({
      ...
      arguments: {
        ...
        body: `This text was created by the local TypeScript content toolkit for ${focus}.`,
      },
    });
    ...
    const updated = await room.agents.invokeTool({
      ...
      arguments: {
        id: proofId,
        headline,
        body: `The room invoked the local TypeScript toolkit for ${focus}, and the toolkit updated ${contentDisplayPath}.`,
      },
    });
